#!/usr/bin/env bash
# Reload waybar with tinty colors (works with both base16 and base24)

TINTY_CURRENT_SCHEME_FILE="$HOME/.local/share/tinted-theming/tinty/artifacts/current_scheme"
WAYBAR_TEMPLATE="$HOME/.config/tinty/templates/waybar/default.mustache"
WAYBAR_ARTIFACT="$HOME/.local/share/tinted-theming/tinty/artifacts/waybar-colors-file.css"

if [ -f "$TINTY_CURRENT_SCHEME_FILE" ] && [ -f "$WAYBAR_TEMPLATE" ]; then
    ...
    if [ -n "$SCHEME_FILE" ] && [ -f "$SCHEME_FILE" ]; then
        ...
        # Generate waybar CSS from template (using | as delimiter to handle / in author names)
        sed -e "s|{{scheme-name}}|$SCHEME_NAME|g" \
            ...
            "$WAYBAR_TEMPLATE" > "$WAYBAR_ARTIFACT"
    fi
fi
